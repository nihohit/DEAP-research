...
for labelIndex = 1:4
    fileName = strcat('../result',num2str(labelIndex),'.csv');
    fid = fopen(fileName, 'w');
    fprintf(fid, header);
    fclose(fid);
   
    for participantsIndex = 1:32
       participantResult =  participantsResults(participantsIndex, labelIndex);
       dlmwrite (fileName, participantResult, '-append');
    end
end
